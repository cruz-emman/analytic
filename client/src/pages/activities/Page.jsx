...
const Page = () => {
    ...
    useEffect(() =>{
      const getAssign = async () =>{
       try {
        const getcase = await axios.get('http://localhost:5000/api/v1/case/get_choice') 
        const getcbma = await axios.get('http://localhost:5000/api/v1/cbma/get_choice') 
        const getchtm = await axios.get('http://localhost:5000/api/v1/chtm/get_choice') 
        const getceis = await axios.get('http://localhost:5000/api/v1/ceis/get_choice') 
        const getcmt = await axios.get('http://localhost:5000/api/v1/cmt/get_choice') 
        const getjhs = await axios.get('http://localhost:5000/api/v1/jhs/get_choice') 
        const getslcn = await axios.get('http://localhost:5000/api/v1/slcn/get_choice') 
    ...
    <div className='w-full h-full p-4'>
        <div className='flex flex-col gap-5 items-center justify-center'>
          <div className='flex flex-col h-[600px] w-full border-2  p-4 items-center  justify-center'>
              <p className='text-3xl font-bold text-gray-400 text-center pb-10'>Usage of the Teachers (Page)</p>
